var core = {
  ...
  "action": {
    ...
    "interface": {
      "close": function (data, sender) {
        const tabId = sender && sender.tab ? sender.tab.id : (data && data.tabId);
        if (tabId !== undefined && tabId !== null) {
          app.page.send("close", null, tabId, null);
        }
      },
      "print": function (data, sender) {
        const tabId = sender && sender.tab ? sender.tab.id : (data && data.tabId);
        if (tabId !== undefined && tabId !== null) {
          app.page.send("print", null, tabId, null);
        }
      }
    },
